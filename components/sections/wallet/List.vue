...
onMounted(async () => {
  ...
  try {
    ...
    const response = await fetch("https://cms.berkahcapitalcredit.com/api/v1/loans?orderBy=updated_at&order=desc&paginate=1&page=1", {
    ...
    const responseWallet = await fetch("https://cms.berkahcapitalcredit.com/api/v1/wallet/withdraw?orderBy=updated_at&order=desc&paginate=1&page=1", {
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }
    });
...
const submitOtp = async () => {
  ...
  const token = getCookie("token");
  try {
    const response = await fetch("https://cms.berkahcapitalcredit.com/api/v1/wallet/withdraw/validate-otp", {
      method: "POST",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
