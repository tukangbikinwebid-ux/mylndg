...
onMounted(async () => {
  try {
    ...
    if (!user.value.anggota?.anggota_detail) {
      window.location.href = "/my-account"; // Redirect jika gagal ambil data
    }

    // if (!user.value.anggota?.anggota_bank) {
    //   window.location.href = "/my-account"; // Redirect jika gagal ambil data
    // }

    if (user.value.anggota?.status == -1) {
      window.location.href = "/my-account"; // Redirect jika gagal ambil data
    }
  } catch (error) {
    console.error(error);
...
<template>
  <div class="min-h-screen relative overflow-hidden">
    ...
    <SectionsLoanForm />
    <CardsNavigasiFooter title="loan" type="menu" />
  </div>
</template>
